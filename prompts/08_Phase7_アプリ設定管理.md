...
## 【最優先】着手前に確認すること：段階反映の前提を検証し直す

2026年7月28日のACL一括付与作業で、`PUT /k/v1/preview/app/acl.json` は
customize.json とは異なり、**テスト環境を経由せず即座に運用環境へ反映される**
ことが実測で判明しました（CLAUDE.md 6-3-1参照）。

以下の `10_apply_fields.js` / `11_deploy_settings.js` の設計は、
「フィールド変更もcustomize.jsonと同じ2段階（テスト環境→deploy）である」
という**未検証の前提**で書かれています。実装前に、検証用アプリで以下を確認してください。

1. 検証用アプリに、影響の無いフィールドを1つ追加する
   （`POST /k/v1/preview/app/form/fields.json`）
2. **deployを呼ばずに**、運用環境 `GET /k/v1/app/form/fields.json`
   （`preview` を付けない方）を取得する
3. 追加したフィールドが運用環境に既に反映されているか確認する
   - 反映されていた → ACLと同様に即時反映される。`10_apply_fields.js` と
     `11_deploy_settings.js` を1本に統合し、deployステップを削除して設計し直す
   - 反映されていなかった → 従来通り2段階。以下の設計のまま進めてよい
4. 検証結果を `docs/01_設計図.md` と CLAUDE.md 6-3 に追記してから、
   本格実装に進むこと

【禁止】この検証を飛ばして、推測でdeployステップの有無を決めること
