...
async function checkApp(appId) {
  console.log('');
  console.log('--- App' + appId + '：github-bot自身でGET /k/v1/app/customize.json ---');
  try {
    const data = await kintone.apiGet('/k/v1/app/customize.json', { app: appId });
...
async function main() {
  ...
  console.log('github-bot自身によるcustomize.json取得確認を開始します（読み取り専用）');
